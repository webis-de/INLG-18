...
def build_vocab(data, path, min_occur=5, source = None):
    ...
    else:   # using other source of word embedding
        with open(source,'r') as embf:
            words = [line.split(' ')[0] for line in embf]
        for word in words:
            word2id[word] = len(word2id)
            id2word.append(word)
        vocab_size = len(word2id)
